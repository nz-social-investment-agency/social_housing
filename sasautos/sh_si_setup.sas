...
%global si_pop_table_out si_id_col si_as_at_date;
%let si_pop_table_out = sh_pop_master_char;          /* this must be the name of the output table in si_get_cohort macro */
%let si_id_col = snz_uid;                         /* the id column name that you join tables on */
%let si_as_at_date = hnz_na_date_of_application_date;         /* the date at which the intervention occured */
...
%let si_num_periods_before = -4;				/* number of periods in the profile window for use in windowing script */
%let si_num_periods_after = 6;					/* number of periods in the forecast window for use in windowing script */
